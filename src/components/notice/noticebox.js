...
export default function Noticebox({ div, navigation }) {
    ...
    // 페이지 데이터를 불러오는 함수
    const fetchPosts = (page) => __awaiter(this, void 0, void 0, function* () {
        setLoading(true);
        try {
            let response;
            if (category === "council") {
                const councilNoticeResult = yield getCouncilNotice();
      ...
      <View style={select.select}>
        <View style={{ flexDirection: "row" }}>
          ...
          <TouchableOpacity onPress={() => onCategoryChange("council")}>
            <Text style={category === "council" ? select.active : select.inactive}>
              자율회
            </Text>
          </TouchableOpacity>
        </View>
        {category === "council" && authState.staff === "YES" && (<TouchableOpacity onPress={handleOpenNewPost}>
            <AntDesign name="form" style={select.writeText}/>
          </TouchableOpacity>)}
      </View>

      <NewPost isVisible={isModalVisible} onClose={handleCloseNewPost} onSuccess={handlePostSuccess} council="true"/>

      <FlatList data={data} renderItem={({ item, index }) => (<TouchableOpacity key={index} onPress={() => {
                if (category === "council") {
          ...
          </TouchableOpacity>)} keyExtractor={(item, index) => item.id.toString()} ListFooterComponent={loading ? <ActivityIndicator /> : null}/>

      <Pagination totalPages={cbhsTotalPages} currentPage={currentPage} onPageChange={onPageChange}/>
    </SafeAreaView>);
}
