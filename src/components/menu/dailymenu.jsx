...
export default function Menucard({ selectedDate }) {
  ...
  const [formattedDate, setFormattedDate] = useState(
    new Date(selectedDate.replace(/\s/g, "-")).toLocaleDateString(
      "ko-KR",
      options
    )
  );
  ...
  useEffect(() => {
    (async () => {
      const data = await getBreakMenuData(
        convertDateToCustomFormat(selectedDate)
      );
  ...
  useEffect(() => {
    (async () => {
      const data = await getLunchMenuData(
        convertDateToCustomFormat(selectedDate)
      );
  ...
  useEffect(() => {
    (async () => {
      const data = await getDinnerMenuData(
        convertDateToCustomFormat(selectedDate)
      );
