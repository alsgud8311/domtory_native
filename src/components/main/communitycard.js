...
export default function CommunityCard({ navigation }) {
    ...
    useFocusEffect(useCallback(() => {
        const getData = () => __awaiter(this, void 0, void 0, function* () {
            try {
                const { success, data } = yield getLatestPosts("1");
                if (success) {
    ...
    return (<View style={styles.container}>
      <View style={styles.description}>
        ...
        <TouchableOpacity style={styles.moreButton} onPress={() => navigation.navigate("자유게시판")}>
          <Text>더 보기</Text>
      ...
      <View style={styles.card}>
        {communityData ? (<View>
            {communityData.map((post, index) => (<TouchableOpacity key={index} onPress={() => navigation.navigate("자유 게시판", {
                    postId: post.id,
                    memberId: post.member,
                })} style={index !== communityData.length - 1
                    ? styles.postItem
                    : styles.postLastItem} // Apply borderBottom only to non-last items
            >
