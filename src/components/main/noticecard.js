...
export default function NoticeCard({ navigation }) {
    ...
    useFocusEffect(useCallback(() => {
        const getData = () => __awaiter(this, void 0, void 0, function* () {
            const { success, data } = yield getNoticePageData("1");
            if (success) {
    ...
    return (<ScrollView style={styles.container}>
      <View style={styles.description}>
        ...
        <TouchableOpacity style={styles.moreButton} onPress={() => navigation.navigate("공지사항", { div: "cbhs" })}>
          <Text>더 보기</Text>
          <AntDesign name="right" size={15}/>
        </TouchableOpacity>
      </View>
      <View style={styles.card}>
        {noticeData ? (noticeData.map((notice, index) => (<TouchableOpacity key={index} onPress={() => openBrowserAsync(notice.notice_url)} style={index !== noticeData.length - 1 ? styles.postItem : null} // Apply borderBottom only to non-last items
        >
