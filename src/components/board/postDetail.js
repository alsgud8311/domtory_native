...
import { SafeAreaView, View, Text, Image, Dimensions, StyleSheet, ScrollView, KeyboardAvoidingView, TextInput, Platform, TouchableOpacity, Alert, RefreshControl, } from "react-native";
...
import { postComment, deleteComment, postReply, deleteReply, report, block, postLike, postBookmark } from "../../server/board";
import { useAuth } from "../../store/AuthContext";
import ImageModal from "react-native-image-modal";
import Hyperlink from "react-native-hyperlink";
import CommentBox from "../postdetail/comment";
export const handleReport = (type, id) => __awaiter(void 0, void 0, void 0, function* () {
...
export default function PostDetail({ data, reloadData, postId }) {
    ...
    const promptForReply = (commentId) => {
        Alert.alert("대댓글 작성", "대댓글을 작성하시겠습니까?", [
            {
                text: "취소",
                style: "cancel",
            },
    ...
    const handleCommentSubmit = () => __awaiter(this, void 0, void 0, function* () {
        const { success } = yield postComment(postId, comment);
        if (success) {
    ...
    // 대댓글 POST
    const handleReplySubmit = () => __awaiter(this, void 0, void 0, function* () {
        if (!currentReplyingTo)
            return;
        const { success } = yield postReply(currentReplyingTo, comment);
        if (success) {
            Alert.alert("댓글이 작성되었습니다.");
            setComment("");
    ...
    // 댓글 삭제
    const handleDelete = (commentId) => __awaiter(this, void 0, void 0, function* () {
        const { success } = yield deleteComment(commentId);
        if (success) {
    ...
    const confirmReplyDelete = (commentId) => {
        Alert.alert("대댓글 삭제", "대댓글을 삭제하시겠습니까?", [
            {
                text: "취소",
                style: "cancel",
            },
    ...
    // 대댓글 삭제
    const handleReplyDelete = (commentId) => __awaiter(this, void 0, void 0, function* () {
        const { success } = yield deleteReply(commentId);
        if (success) {
    ...
    // 신고
    const handleReport = (type, id) => __awaiter(this, void 0, void 0, function* () {
        const result = yield report(type, id);
        if (result.success) {
    ...
    const handleBlock = (commentId) => {
        Alert.alert("댓글/대댓글 차단", "해당 댓글/대댓글을 차단하시겠습니까? 차단하면 해당 댓글은 모두에게 보이지 않습니다.", [
            {
                text: "취소",
                style: "cancel",
            },
    ...
    const commentBlock = (commentId) => __awaiter(this, void 0, void 0, function* () {
        const { success } = yield block(commentId, "comment");
        if (success) {
    ...
    const handlePostLike = () => __awaiter(this, void 0, void 0, function* () {
        if (isLiked) {
            ...
            Alert.alert("좋아요", "이 게시물에 좋아요를 누르시겠습니까?", [
                ...
                },
                {
                    ...
                    onPress: () => __awaiter(this, void 0, void 0, function* () {
                        try {
                            const { success, data } = yield postLike(postId);
                            if (success) {
                                console.log(data);
                                setLikesCount(data.likes_cnt);
                                setIsLiked(true);
    ...
    const handlePostBookmark = () => __awaiter(this, void 0, void 0, function* () {
        if (isBookmarked) {
            Alert.alert("북마크 취소", "북마크를 취소하시겠습니까?", [
                ...
                },
                {
                    ...
                    onPress: () => __awaiter(this, void 0, void 0, function* () {
                        try {
                            const { success, data } = yield postBookmark(postId);
                            if (success) {
                                console.log(data);
                                setBookmarkCount(data.bookmark_cnt);
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollView} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh}/>}>
        ...
        {data &&
            ...
            data.post_image.map((img, index) => {
                if (!img.is_deleted) {
                    return (<ImageModal resizeMode="contain" key={img.id} source={{ uri: img.image_url }} style={{
                            width: screenWidth,
        ...
        <View style={styles.comment}>
          <TouchableOpacity onPress={handlePostLike} style={{
            display: "flex",
            flexDirection: "row",
            alignItems: "center",
        }}>
          ...
          <TouchableOpacity onPress={handlePostBookmark} style={{
            display: "flex",
            flexDirection: "row",
            alignItems: "center",
        }}>
            <FontAwesome5 name="bookmark" size={16} color={isBookmarked ? "blue" : "gray"} style={{ marginRight: 4, paddingTop: 2 }}/>
            <Text style={{ fontSize: 15, color: isBookmarked ? "blue" : "gray" }}>{bookmarkCount}</Text>
          </TouchableOpacity>
        </View>
        <CommentBox data={data} setCurrentReplyingTo={setCurrentReplyingTo} reloadData={reloadData}/>
      </ScrollView>
      {/* 댓글 작성 */}
      <KeyboardAvoidingView style={styles.writeComment} behavior={Platform.OS === "ios" ? "padding" : null} keyboardVerticalOffset={Platform.OS === "ios" ? 100 : -500} // iOS 외 플랫폼에서는 원하는 값으로 설정
    >
        <View style={styles.inputBox}>
          <TextInput placeholder={currentReplyingTo ? "대댓글을 입력하세요" : "댓글을 입력하세요"} placeholderTextColor={"#848484"} multiline={true} scrollEnabled={true} onChangeText={onChangeComment} value={comment} style={styles.commentInput}/>
