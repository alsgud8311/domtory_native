...
import { StyleSheet, FlatList, TouchableOpacity, SafeAreaView, RefreshControl, Alert, View, Text, } from "react-native";
...
export default function Board({ boardId, navigation }) {
    ...
    const fetchData = () => __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const response = yield getPostList(boardId, currPage);
            ...
            if (currPage === 1) {
                ...
                setData((prevData) => {
                    ...
                    const newData = response.data.postList.filter((newItem) => !prevData.some((prevItem) => prevItem.id === newItem.id));
                    // 이전 데이터와 새로운 데이터를 합침
    ...
    return (<SafeAreaView style={styles.container}>
      <FlatList refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh}/>} data={data} renderItem={({ item }) => (<PostListItems item={item} navigation={navigation} setGoBackRefresh={setGoBackRefresh}/>)} keyExtractor={(item) => item.id} contentContainerStyle={{ paddingVertical: 20 }} onEndReached={handleLoadMore} onEndReachedThreshold={0.5}/>
      {boardId <= 5 && (<>
