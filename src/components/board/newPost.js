...
export default function NewPost({ isVisible, onClose, boardId, onPostSubmit, council, onSuccess, post, }) {
        ...
        Alert.alert("업로드 하시겠습니까?", "", [
            { text: "아니오", style: "cancel" },
            {
                ...
                onPress: () => __awaiter(this, void 0, void 0, function* () {
                    ...
                    try {
                        if (council === "true") {
                            const { success } = yield writeCouncilPost(formData);
                            if (success) {
                            ...
                            const { success } = yield writePost(boardId, formData);
                            if (success) {
        ...
        Alert.alert("수정하시겠습니까?", "", [
            { text: "아니오", style: "cancel" },
            {
                ...
                onPress: () => __awaiter(this, void 0, void 0, function* () {
                    ...
                    try {
                        const result = yield updatePost(post.id, formData);
    ...
    return (<Modal animationType="slide" transparent={false} visible={isVisible} onRequestClose={handleClose}>
      <SafeAreaView style={styles.safeArea}>
        <TouchableWithoutFeedback onPress={dismissKeyboard}>
          <ScrollView style={styles.container}>
            ...
            {/* 제목 */}
            <TextInput style={isTitleFocused ? styles.titleFocused : styles.titleNotFocused} spellCheck={false} autoCorrect={false} onFocus={() => setIsTitleFocused(true)} onBlur={() => setIsTitleFocused(false)} selectionColor="#ffa551dc" onChangeText={onChangeTitle} value={title} placeholder={"제목"} placeholderTextColor={"#959595"} multiline={true}/>
            {/* 내용 */}
            <TextInput spellCheck={false} autoCorrect={false} style={isContentFocused ? styles.focused : styles.inputContent} onFocus={() => setIsContentFocused(true)} onBlur={() => setIsContentFocused(false)} selectionColor="#ffa551dc" onChangeText={onChangeContent} value={content} placeholder={"내용"} placeholderTextColor={"#959595"} multiline={true}/>
            {/* 카메라, 완료버튼 */}
            ...
            color: "white",
            fontWeight: 800,
        }} onPress={() => openBrowserAsync("https://luxurious-share-af6.notion.site/ebd8c0c297574790921bea4d47d7b73f?pvs=4")}>
                  커뮤니티 이용규칙 전문 보기
                </Text>
              </TouchableOpacity>
            </ScrollView>
