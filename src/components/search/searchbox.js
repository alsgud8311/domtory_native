...
import { View, Text, StyleSheet, TouchableOpacity, TextInput, FlatList, Keyboard, Image, Alert, // 알림창을 사용하기 위해 추가
 } from "react-native";
...
export default function Searchbox({ route, navigation }) {
    ...
    const renderItem = ({ item, board }) => {
        ...
        return (<TouchableOpacity onPress={navigateToDetailScreen}>
        <View style={styles.item}>
          <View style={{ flexDirection: "column", marginBottom: 5 }}>
            <View>
              {board == "전체 게시판" && (<Text style={styles.board}>{Boardname[item.board]}</Text>)}
    ...
    const handleSearch = () => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const searchData = yield getSearhedData(inputList, Boardname[board]);
            setData(searchData);
    ...
    return (<View style={styles.container}>
      <View style={styles.searchcontainer}>
        <View style={styles.inputContainer}>
          <TextInput ref={textInputRef} style={styles.searchbox} placeholder="검색어를 입력하세요." onChangeText={handleInputChange} value={inputText} onSubmitEditing={() => handleSearch()}/>
        ...
        <TouchableOpacity onPress={hideKeyboard} style={{ alignItems: "center", flex: 1 }}>
          <Text style={styles.canceltext}>취소</Text>
        </TouchableOpacity>
      </View>

      <View>
        {data ? (<FlatList data={data} renderItem={({ item }) => renderItem({ item, board })} keyExtractor={(item) => item.id}/>) : (<View style={styles.empty}>
            <AntDesign name="search1" size={30} color="gray"/>
            <Text style={styles.emptyText}>{board}의 글을 검색해보세요</Text>
          </View>)}
        {data && data.length === 0 && (<View style={styles.empty}>
