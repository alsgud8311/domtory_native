...
export const pushRenderItems = ({ item, navigation }) => {
    ...
    const navigateToDetailScreen = () => __awaiter(void 0, void 0, void 0, function* () {
        if (item.boardId && item.postId) {
            const { success } = yield pushCheckUpdate(item.memberId, item.pushedAt);
            if (success) {
    ...
    return (<TouchableOpacity key={item.index} style={{ flex: 1, backgroundColor: "white" }} onPress={navigateToDetailScreen}>
      <View style={[
            styles.item,
            item.isChecked
