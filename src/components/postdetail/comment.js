...
export default function CommentBox({ data, setCurrentReplyingTo, reloadData }) {
    ...
    const handlePostLike = (comment) => () => __awaiter(this, void 0, void 0, function* () {
            ...
            Alert.alert('좋아요', '이 댓글에 좋아요를 누르시겠습니까?', [
                ...
                },
                {
                    ...
                    onPress: () => __awaiter(this, void 0, void 0, function* () {
                        try {
                            const { success, data } = yield postCommentLike(comment.id);
                            if (success) {
                                setLikesCounts(prevCounts => (Object.assign(Object.assign({}, prevCounts), { [comment.id]: data.likes_cnt })));
                                setIsLiked((prevIsLiked) => (Object.assign(Object.assign({}, prevIsLiked), { [comment.id]: true })));
                            }
                        }
                        catch (error) {
    ...
    return (<>
      {data.comment &&
            ...
            data.comment.map((comment) => comment.is_deleted && comment.reply.length === 0 ? null : (<View key={comment.id} style={styles.commentContainer}>
              <View style={{
                    flexDirection: "row",
                    justifyContent: "space-between",
                }}>
                <View style={{ flexDirection: "row" }}>
                  <Image source={domtory} style={{
                    width: 23,
                    height: 23,
                    borderRadius: 3,
                    alignItems: "center",
                  ...
                  {/* 댓글의 상태 검사 후 필터링 */}
                  {comment.is_deleted ? (<Text style={styles.commentMember}>삭제된 도토리</Text>) : comment.is_blocked ? (<Text style={styles.commentMember}>유배당한 도토리</Text>) : comment.anonymous_number === 0 ? (<Text style={styles.commentMember}>글쓴이 도토리</Text>) : (<Text style={styles.commentMember}>
                ...
                <View style={styles.commentOption}>
                  ...
                  {parseInt(authState.id) === comment.member ? (<TouchableOpacity onPress={() => confirmDelete(comment.id)}>
                      <Octicons name="trash" style={styles.commnetDelete}/>
                    </TouchableOpacity>) : authState.staff === "YES" ? (<>
                      <Octicons name="stop" style={styles.commnetReport} onPress={() => confirmAndReport("comment", comment.id)}/>
                      <FontAwesome5 name="ban" style={styles.commnetReport} onPress={() => handleBlock(comment.id)}/>
                    </>) : (<Octicons name="stop" style={styles.commnetReport} onPress={() => confirmAndReport("comment", comment.id)}/>)}
                </View>
              </View>
              {comment.is_deleted || comment.is_blocked ? (<Text style={styles.commentDeleted}>
                  {comment.is_deleted
                        ? "삭제된 댓글입니다."
              ...
              {/* 대댓글 렌더링 부분 */}
              {comment.reply && comment.reply.length > 0 && (<View style={styles.replyContainer}>
                  <ReplyCommentBox comment={comment} reloadData={reloadData} commentBlock={commentBlock} handleBlock={handleBlock} handleReport={handleReport} handleReplyDelete={handleReplyDelete} confirmAndReport={confirmAndReport} confirmReplyDelete={confirmReplyDelete}/>
                </View>)}
            </View>))}
    </>);
}
