...
const CommunityStack = ({ navigation }) => {
    ...
    return (<Stack.Navigator initialRouteName="커뮤니티 바로가기 탭" screenOptions={stackScreenOptionsWithTitle}>
      <Stack.Screen name="커뮤니티 바로가기" component={CommunityShortCuts} options={{
            title: "게시판",
      ...
      <Stack.Screen name="스크랩한 글 보기" component={BookmarkDetail} options={({ navigation }) => ({
      ...
      <Stack.Screen name="취준생 게시판" component={Jobseeker} options={{
            headerRight: () => {
                return (<AntDesign name="search1" size={30} color="black" onPress={() => navigation.navigate("검색", { board: "취준생 게시판" })}/>);
            },
        }}/>
      <Stack.Screen name="번개모임게시판" component={ImpromptuDetail} options={({ navigation }) => ({
            headerRight: () => <Popup navigation={navigation}/>,
        })}/>
      <Stack.Screen name="번개모임 게시판" component={Impromptu} options={{
            headerRight: () => {
                return (<AntDesign name="search1" size={30} color="black" onPress={() => navigation.navigate("검색", { board: "번개모임 게시판" })}/>);
            },
        }}/>
      <Stack.Screen name="중고거래게시판" component={FleeMarketDetail} options={({ navigation }) => ({
            headerRight: () => <Popup navigation={navigation}/>,
        })}/>
      <Stack.Screen name="중고거래 게시판" component={FleeMarket} options={{
            headerRight: () => {
                return (<AntDesign name="search1" size={30} color="black" onPress={() => navigation.navigate("검색", { board: "중고거래 게시판" })}/>);
            },
        }}/>
      <Stack.Screen name="분실물게시판" component={LostAndFoundDetail} options={({ navigation }) => ({
            headerRight: () => <Popup navigation={navigation}/>,
        })}/>
      <Stack.Screen name="분실물 게시판" component={LostAndFound} options={{
            headerRight: () => {
                return (<AntDesign name="search1" size={30} color="black" onPress={() => navigation.navigate("검색", { board: "분실물 게시판" })}/>);
            },
        }}/>
